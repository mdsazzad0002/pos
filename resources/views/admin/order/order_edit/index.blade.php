...
<div class="card">
    ...
    <form class="card-body purchause_create" method="POST" action="{{ route('admin.order.update',$order->id) }}">
        @csrf
        @method('put')
        <div class="row mb-3">
            <div class="col-md-6 mb-2">
                <div class="input-group">
                    <select  name="customerID" required class="form-control select2 input-group-prepend" data-ajax="true" data-url="{{ route('admin.customer.select') }}" id="">
                        <option selected value="{{ $order->customer_id }}"> {{ $order->customer->name ?? '' }} </option>
                    </select>
                    @can('customer create')

                    <button class="btn btn-primary input-group-append" data-dialog="  modal-dialog-scrollable modal-dialog-centered" onclick="button_ajax(this)" data-title="Add New  supplier" data-href="{{ route('admin.customer.create') }}">+ Add New customer</button>
                    @endcan
      ...
      function format_query_result(data_items){
        ...
        Object.keys(data_items).forEach(function(data){
            product_filter +=`<tr class="single_item_result ">
                ...
                <td class="name p-1">
                    ${data_items[data].price}
                </td>
                <td class="action p-1">
                    <button type="button" class="btn btn-primary" onclick="items_add_from_quey(${data_items[data].id}, '${data_items[data].text}', ${data_items[data].price})">+</button>
                </td>
            </tr>`;
        })
            ...
            if ($('#scan_match_andInput').is(':checked')) {
                $.ajax({
                    ...
                    success:function(data){
                        ...
                        if(data != null){
                            items_add_from_quey(data.id, data.text, data.price);
                        }else{
        ...
        function items_add_from_quey(id, name, price){
            var data_format = ` <tr class="curent_parents_data items${id}">
                            ...
                            <td>
                                <input disabled oninput="calclute_inline('.items${id}')" type="numberr" class="price" name="price[]" value="${price}">
                            </td>
                            <td>
