<form
    @if ($subcategory) action="{{ route('admin.subcategory.update', $subcategory->id) }}"
@else
    action="{{ route('admin.subcategory.store') }}" @endif
    ...
    <select type="text" name="category" data-url="{{ route('admin.category.select') }}" data-ajax="true"
        class="form-control input-group-prepend select2" placeholder="Username" aria-label="Username"
        aria-describedby="basic-addon1">
        <option value="{{ $subcategory ? $subcategory->category->id : 0 }}">
            {{ $subcategory ? $subcategory->category->name : '' }}</option>
    </select>
    </div>
    ...
    <div class="mb-2">
        ...
        <input type="text" name="name" class="form-control mb-2"
            value="{{ $subcategory ? $subcategory->name : '' }}" placeholder="Enter your name"
    ...
    <div class="mb-2">
        ...
        <input type="text" name="slug" id="slug" class="form-control mb-2"
            value="{{ $subcategory ? $subcategory->slug : '' }}" placeholder="Enter your slug">
    ...
    <div class="mb-2">
        ...
        <input type="checkbox" name="need_additional" id="need_additional_check" class=" mb-2" value="1"
            {{ $subcategory ? ($subcategory->need_additional == 1 ? 'checked' : '') : '' }}
            onchange="document.getElementById('additional_info').style.display = this.checked ? 'block' : 'none'">
    </div>

    <div class="mb-2" id="additional_info"
        style="display: {{ $subcategory ? ($subcategory->need_additional == 1 ? 'block' : 'none') : 'none' }}">
        <div class="mb-2">
        ...
        <div class="col-lg-12">
            ...
            <textarea type="text" name="primary_description" class="form-control mb-2 summernote" placeholder="Description">{{ $subcategory ? $subcategory->primary_description : '' }}</textarea>
        </div>
    ...
    <label for="status"> Subcategory Status <br />
        ...
        <input type="checkbox" class="toggle" placeholder=" Status" name="status"
            {{ $subcategory ? ($subcategory->status == 1 ? 'checked' : '') : '' }} id="status" value="1">
    </label>
    ...
    <div class="">
        <label type="button" onclick="upload_select(this)"> Image <br>
            <input type="text" name="image" id="image"
                value="{{ $subcategory ? $subcategory->upload_id : 0 }}" class="form-control mb-2" hidden>
            <img style="max-height: 60px" src="{{ dynamic_asset($subcategory ? $subcategory->upload_id : 0) }}"
                alt="">
        </label>
    </div>
    </div>
