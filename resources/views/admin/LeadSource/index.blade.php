...
{{--  Define Site Title  --}}
@section('title', settings('LeadSources', 10))
    ...
    <div class="card-header d-flex align-items-center justify-content-between">
        ...
        <div>
            @can('LeadSource create')
                <button class="btn btn-primary" onclick="button_ajax(this)" data-dialog=" modal-dialog-scrollable modal-dialog-centered" data-title="Add New  LeadSource" data-href="{{ route('admin.LeadSource.create') }}">+ Add New LeadSource</button>
            @endcan
        </div>
    </div>
    <div class="card-body">
        <table id="LeadSource" class="table table-bordered table-striped table-hover">
            <thead>
                <th>
                    SI
                </th>
...
@push('js')
<script>
    var datatableM =  $('#LeadSource').DataTable({
        serverSide:true,
        responsive: true,
        processing:true,
        ajax:'',
