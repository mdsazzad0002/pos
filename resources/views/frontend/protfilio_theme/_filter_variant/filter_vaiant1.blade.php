        ...
        @php
           $categories = App\Models\Category::where('status', 1);
            ...
            if (settings('service_show_status', 80) == 1 && settings('product_show_status', 80) == 1) {
                ...
                $categories = $categories->where('service_status', 1);
            } elseif (settings('product_show_status', 80) == 1 || settings('service_show_status', 80) == 0) {
                $categories = $categories->where('service_status', 0);
            } else {
                $categories = $categories->whereNot('service_status', 0);
                $categories = $categories->whereNot('service_status', 1);
            }
            $categories = $categories->orderBy('position_order', 'asc');
                        ...
                        <div class="sidebar bg-white">
                            <form action="" class="mb-24 searcform">
                                <input type="text" name="search" id="searchBar" class="form-control"
                                    value="{{ $_GET['q'] ?? '' }}" placeholder="Search Here...">
                                <button>
                                                    ...
                                                    <li class="checkbox-group">
                                                        <div
                                                            ...
                                                            @if (count($category->subcategories_info) > 0)
                                                                <i
                                                                    class="fa-thin fa-chevron-down fw-500 light-gray"></i>
                                                            @endif
                                                        </div>
                                                        @if (count($category->subcategories_info) > 0)
                                                                    ...
                                                                    <li class="d-flex align-items-center gap-2 mb-12">
                                                                        <div class="check-block">
                                                                            <input type="checkbox"
                                                                                ...
                                                                                class="sub_check_category"
                                                                                data-type="subcategory"
                                                                                id="sub{{ $subcategory->id }}"
                                                                                name="subcategory[]">
                                                                            <label
                                                                                for="sub{{ $subcategory->id }}">{{ $subcategory->name }}</label>
                                                                        </div>
                                            ...
                                            <div class="wrapper">
                                                ...
                                                <div class="range-input">
                                                    ...
                                                    <input type="range" class="range-max" min="0"
                                                        max="100000" value="1000000" step="100">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <ul class="list-unstyled">
                                                @foreach ($brand_items as $brand)
                                                    <li class="d-flex align-items-center justify-content-between mb-12">
                                                        <div class="check-block">
                                                            <input type="checkbox" class="brands_input"
                                                                value="{{ $brand->id }}"
                                                                id="brand{{ $brand->id }}">
                                                            <label
                            ...
                            @if (settings('show_filter_weight_product_status', 80) == 1)
                                {{-- Filter by brand --}}
                                @php
                                    $weight_items = \App\Models\product::where('status', 1)
                                        ->distinct()
                                        ->pluck('weight');
                                        ...
                                        <div class="content-block">
                                            <ul class="list-unstyled">
                                                @foreach ($weight_items as $weight)
                                                    <li class="d-flex align-items-center justify-content-between mb-12">
                                                        <div class="check-block">
                                                            <input type="checkbox" class="weight_input"
                                                                value="{{ $weight }}"
                                                                id="weight{{ $weight }}">
                                                            <label
                                                                for="weight{{ $weight }}">{{ $weight }}</label>
                                                        </div>
                                                        <p class="light-gray fw-400"></p>
                                                    </li>
                                                @endforeach
                                    ...
                                    <div class="content-block">
                                        <div class="d-flex justify-content-between align-items-center mb-16">
                                            <div class="star-rating flex-column w-100">
                                                ...
                                                <div class="d-flex align-items-center justify-content-between mb-2">
                                                    <input type="radio" id="1-stars" name="rating"
                                                        value="1">
                                                ...
                                                <div class="d-flex align-items-center justify-content-between mb-2">
                                                    <input type="radio" id="0-stars" name="rating"
                                                        value="0">
                            ...
                            @if (settings('show_filter_feature_product_status', 80) == 1)
                                @php
                                    $feature_product = \App\Models\product::where('status', 1)
                                        ->where('feature', 1)
                                        ->orderBy('id', 'desc')
                                        ->limit(5)
                                        ->get();
                                    $filter_page = \App\Models\page::where('status', 1)
                                        ->where('page_type', 'view')
                                        ->first();
                                @endphp
                        ...
                        <div class="shop-grid-topbar mb-16">
                            <div class="first-block">
                                ...
                                <div class="results d-lg-block">
                                    <p class="dark-gray">Showing <span class="v_first">0</span> - <span
                                            class="v_last">0</span> of <span class="v_total">0</span> Results</p>
                                </div>
                            </div>
