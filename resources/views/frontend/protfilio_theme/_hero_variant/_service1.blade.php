    ...
    <div class="filter_form">
        <form action="{{ url($filter_page->slug) }}" method="get" class="d-flex flex-nowrap">
            <div class="location">
                ...
                <button type="button" data-bs-toggle="modal" data-bs-target="#locationSelectModal"><i class="bi bi-geo-alt"></i> <span class="location_name">Location</span></button>
            </div>
            <div class="search">
...
<div class="category_container">
  ...
  <div class="row_grid">
    @foreach (category() as $category)
      ...
      <a class="cat-link"  onclick="modal_category_open('#v-pills-{{ $category->slug }}', '#locationSelectModal_category')">
...
@endphp
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <div class="row">
                ...
                <div class="col-md-6 col-xl-4 single_area" onclick="change_area({{ $area->id }}, '{{ $area->name }}')">
                    {{ $area->name }}
                </div>
                @endforeach
            </div>
  ...
  <div class="modal fade" id="locationSelectModal_category" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
          ...
          <div class="d-flex align-items-start">
            <div class="nav flex-column nav-pills " id="v-pills-tab" role="tablist" aria-orientation="vertical">
              @foreach (category() as $key => $category)
                  <button class="nav-link category_btn @if ($key == 0) active  @endif " id="v-pills-{{ $category->slug }}-tab" data-bs-toggle="pill" data-bs-target="#v-pills-{{ $category->slug }}" type="button" role="tab"     aria-controls="v-pills-{{ $category->slug }}" aria-selected="true">
                    <img src="{{ dynamic_asset($category->upload_id) }}" alt=""> {{ $category->name ?? ''}}</button>
              @endforeach
            
            </div>
            <div class="tab-content" id="v-pills-tabContent">
              @foreach (category() as $key => $category)
                  <div class="tab-pane fade  @if ($key == 0) show active  @endif " id="v-pills-{{ $category->slug }}" role="tabpanel" aria-labelledby="v-pills-{{ $category->slug }}-tab">
                    {{-- <div class="tab-pane fade show active" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab">...</div> --}}

                  
                          @foreach ($category->subcategories_info as $sub_category)
                          <a class="csingle_area_sub" href="{{ url($filter_page->slug) }}?subcategory_ids[]={{ $sub_category->id }}&category={{ $category->slug }}" >
                              <img src="{{ dynamic_asset($sub_category->upload_id) }}" alt="" />{{ $sub_category->name }}
                          </a>
                          @endforeach
...
{{-- End Category Sub Category --}}
  <script>
    ...
    function updateButtons() {
        ...
        rightButton.style.opacity =   (container.scrollLeft + container.clientWidth < container.scrollWidth ) ? 1  : 0;
    }
