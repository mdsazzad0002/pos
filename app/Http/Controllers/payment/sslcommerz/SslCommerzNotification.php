...
class SslCommerzNotification extends AbstractSslCommerz
{
    ...
    private $base_sandbox = 'https://sandbox.sslcommerz.com';
    private $base_live = 'https://securepay.sslcommerz.com';
    /**
    ...
    public function __construct()
    {
        ...
        if($payment_credentials){
            ...
            $this->setStoreId($payment_credentials->store_id);
            $this->setStorePassword($payment_credentials->store_password);
        }
    }
    ...
    protected function validate($merchant_trans_id, $merchant_trans_amount, $merchant_trans_currency, $post_data)
    {
        ...
        if (!empty($merchant_trans_id) && !empty($merchant_trans_amount)) {
            ...
            $requested_url = ($this->base_url . $this->config['apiUrl']['order_validate'] . "?val_id=" . $val_id . "&store_id=" . $store_id . "&store_passwd=" . $store_passwd . "&v=1&format=json");

            $handle = curl_init();
            curl_setopt($handle, CURLOPT_URL, $requested_url);
            curl_setopt($handle, CURLOPT_RETURNTRANSFER, true);

            if ($this->sandbox_status) {
    ...
    public function makePayment(array $requestData, $type = 'checkout', $pattern = 'json')
    {
        ...
        $this->setApiUrl($this->base_url . $this->config['apiUrl']['make_payment']);
        ...
        $response = $this->callToApi($this->data, $header, $this->sandbox_status);
